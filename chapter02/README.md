...
* 指向指针的引用
    ...
    r = &i; // r 引用了一个指针，因此给 r 赋值&i 就是令p指向i
    *r = 0; // 解引用 r 得到 i，也就是p指向的对象，将 i 的值改为 0

    要理解上的类型到底是什么，最简单的办法是从右向左阅读r的定义。离变量名最近的符 号(此例中是&r的符号&)对变量的类型有最直接的影响，因此r是 一个引用。声明符的其余部分用以确定r引用的类型是什么，此例中的符号*说明r引用的是一个指针。最后， 声明的基本数据类型部分指出r引用的是一个int 指针。
    ```

* 练习 2.25 说明下列变量的类型和值
    ```
    a. int* ip, i, &r = i; // ip 是一个 int 型指针， i 是一个 int 型变量，r 是一个 int 型引用
    b. int i, *ip = 0; // i 是一个 int 型变量， ip 是一个 int 型指针初始值为空
    c. int *ip, ip2; // ip 是一个 int 型指针，ip2 是一个 int 型变量
    ```
# 2.4 const 限定符
...
* 试试这样想吧：所谓指向常量的指针或引用，不过是指针或引用 ”自以为是“罢了，它们觉得自己指向了常量，所以自觉地不去改变所指对象的值。(绷不住了)
...
* 练习 2.29 假设已有上一个练习中定义的哪些变量，则下面的哪些语句是合法的？请说明原因。
